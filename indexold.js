        ...
        try {
            ...
            if (respuesta && respuesta != undefined) {
                switch (respuesta.status) {
                    case 100:
                        respuesta.informacion = "Flujo completo"
                        break;
                    case 401:
                        respuesta.informacion = "NSS o CURP incorrectos"
                        break;
                    case 402:
                        respuesta.informacion = "Error en busqueda de página del IMSS"
                        break;
                    case 301:
                        respuesta.informacion = "Error de la página durante busqueda"
                        break;
                    default:
                        respuesta.informacion = "Error interno"
                        break;
                }
            } else {
                respuesta.informacion = "Error interno"
...
async function guardaDatosExtraidos(idUnico, datos) {
    return new Promise((resolve, reject) => {
        pool.getConnection((error, connection) => {
            ...
            const query = 'UPDATE registros_SemanasCotizadas SET fechaAlta = ?, fechaBaja = ?, salarioBase  = ?, ultimoPatron = ?, ultimoRegistroPatronal = ? where idRPA=?;';

            connection.query(query, [datos.fechaAlta, datos.fechaBaja, datos.salarioBase, datos.ultimoPatron, datos.ultimoRegistroPatronal, idUnico], (error, results) => {
                connection.release();
